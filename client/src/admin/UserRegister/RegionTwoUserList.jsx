...
const RegionTwoUserList = () => {
  ...
  const fetchApplications = async () => {
    try {
      ...
      return response.data.Region2Data;
    } catch (error) {
  ...
  const exportToExcel = () => {
    const simplifiedData = paginatedUsers.map(user => ({
      ...
      "Account Status": `${user.is_rejected} ${user.rejected_reason ? `- ${user.rejected_reason}` : ""}`,
      "Update By": user.updatedBy,
    }));
    const ws = XLSX.utils.json_to_sheet(simplifiedData);
    const wb = XLSX.utils.book_new(); 
    XLSX.utils.book_append_sheet(wb, ws, 'Sheet1'); 

    XLSX.writeFile(wb, 'Region2Users.xlsx');
    XLSX.writeFile(wb, 'Region2Users_Page_' + currentPage + '.xlsx');
        ...
        <div>
          <div className="box_header">
            <div style={{ padding: "5px 0px" }}>
              ...
              Region-2 Users: <b>{data.length}</b>
            </div>
          </div>
          <div>
                    ...
                    {paginatedUsers.length > 0 ? (
                      paginatedUsers.map((app, index) => (
                        <tr key={app.application_no} className="bg-success text-white">
                          ...
                          <td><span>{app.modified_by}</span><span>{app.date_modified}</span></td>
                          <td>
